...
Describe 'Update-AzHybridBenefit Parameter Validation' {
    ...
    Context 'Mode Parameter' {
        ...
        It 'Should default to Both when not specified' {
            # Override mocks so the workflow reaches Set-HybridBenefitOnVMs
            Mock Get-TargetSubscriptions {
                @([PSCustomObject]@{ Id = 'sub-1'; Name = 'Test Sub' })
            }
            ...
            Invoke-AzHybridBenefitUpdate -WhatIf
            
            Assert-MockCalled Set-HybridBenefitOnVMs -Times 1 -ParameterFilter { $Mode -eq 'Both' }
        }
    }
